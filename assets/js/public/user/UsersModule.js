            ...
            if (secureFile) {
              ...
              if (changeEvent.target.files[0].size < 1000000) {
                ...
                if (identifier.hasClass("gallery")) {
                  fd['model'] = 'gallery';
                  fd['position'] = $(identifier).val();
                } else if (identifier.hasClass("profile")) {
                  fd['model'] = 'profile';
                }
                fd['file'] = data.replace(/^data:image\/(png|jpg|jpeg);base64,/, "");
                // Upload Image
                $http.post('/user/gallery/photos', fd, {}).then(function successCallback(response) {
  ...
  if (url != null && url !== undefined) {
    ...
    for (var serv in services) {
      ...
      if (match) {
        ...
        var urlsrc = "";
        if (serv == "hudl") {
          urlsrc = services[serv].frame + match[1] + "/" + match[2];
